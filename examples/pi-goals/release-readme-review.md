...
!`tag=$(git describe --tags --abbrev=0 2>/dev/null || true); if [ -n "$tag" ]; then git diff --name-status "$tag"..HEAD -- README.md package.json .pi/extensions/goal .pi-goals; else git diff --name-status -- README.md package.json .pi/extensions/goal .pi-goals; fi`
</changed_public_files_since_last_release>
